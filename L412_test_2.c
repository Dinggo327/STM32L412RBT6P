...
void sendColor(uint8_t g, uint8_t r, uint8_t b)
{
    ...
    b = (uint8_t)((float)b * BRIGHTNESS_SCALE);
    sendByte(g);
    sendByte(r);
    sendByte(b);
}
...
void setAllLEDsColor(uint8_t g, uint8_t r, uint8_t b, uint8_t numLeds)
{
...
int main(void)
{
    ...
    while (1)
    {
      ...
      /*    setAllLEDsColor(255, 0, 0);
          ...
          HAL_Delay(1000);                        */
        for (int numLeds = MIN_NUM_LEDS; numLeds <= MAX_NUM_LEDS; numLeds++)
        {
            setAllLEDsColor(100, 0, 0, numLeds);
            HAL_Delay(1000);
            //  setAllLEDsColor(0, 255, 0, numLeds);
            //  HAL_Delay(200);
            //  setAllLEDsColor(0, 0, 255, numLeds);
            //  HAL_Delay(500);
        }
    }
    /* USER CODE END 3 */
